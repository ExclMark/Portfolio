...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    localStorage.setItem("previousProject", 3)
    ...
    function project(span) {
        if (loaded) {
            textElement.innerHTML = `<span class='pwd'>./projects/tictactoe - [${page}/${totalPages}]</span>\n\n`;
            ...
            for (let i = 0; i < text.length; i++) {
                if (text[i] == "|" && !bold) {
                    ...
                    let url = localization.project.tictactoe.link;
                    url = url.replace(/~/g, '');
            ...
            if (currentMenuIndex < text.length) {
                ...
                if (text[currentMenuIndex] === "~" && !link) {
                    ...
                    span.className = 'link';
                    span.href = localization.project.tictactoe.link.replace(/~/g, '');
                    textElement.appendChild(span);
                    link = true;
    ...
    function updateHeader() {
        ...
        text += localization.project.name + " " + localization.project.tictactoe.name + "\n";
        text += localization.project.desc + " " + localization.project.tictactoe.desc + "\n";
        text += localization.project.tech + " " + localization.project.tictactoe.tech + "\n";
        text += localization.project.link + " " + localization.project.tictactoe.link + "\n";
        ...
        span.className = 'pwd';
        textElement.appendChild(span);
        span.textContent = `./projects/tictactoe - [${page}/${totalPages}]\n\n`;
        currentMenuIndex = 0;
        loaded = false;
        project();
    }
